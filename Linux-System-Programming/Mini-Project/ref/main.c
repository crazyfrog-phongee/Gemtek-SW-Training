...
void check_rv(int status)
{
    switch (status)
    {
    case EAI_ADDRFAMILY:
    case EAI_AGAIN:
        LOG(TAG, "The name server returned a temporary failure indication. Try again later.");
        break;

    case EAI_BADFLAGS:
        LOG(TAG, "hints.ai_flags contains invalid flags; or, hints.ai_flags included AI_CANONNAME and name was NULL.");
        break;
    case EAI_FAIL:
        LOG(TAG, "The name server returned a permanent failure indication.");
        break;
    case EAI_FAMILY:
        LOG(TAG, "The requested address family is not supported.");
        break;
    case EAI_MEMORY:
        LOG(TAG, "Out of memory.");
        break;
    case EAI_NODATA:
        LOG(TAG, "The specified network host exists, but does not have any network addresses defined.");
        break;
    case EAI_NONAME:
        LOG(TAG, "The node or service is not known; or both node and service are NULL;");
        break;
    case EAI_SERVICE:
        LOG(TAG, "The requested service is not available for the requested socket type.");
        break;
    case EAI_SOCKTYPE:
        LOG(TAG, "The requested socket type is not supported.");
        break;
    case EAI_SYSTEM:
        LOG(TAG, "Other system error; errno is set to indicate the error.");
        break;
